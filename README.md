...
本仓库绝大部分代码与实现均由 AI 生成，且没有经过完整的人工 review。自行承担使用风险。

The vast majority of the code and implementation in this repository were generated by AI and have not undergone complete human review. Use it by your own risk.

# ZTE W132D 主线 Linux 移植

本仓库包含 ZTE Cloud Computer W132D（Rockchip RK3528）的公开板级描述、补丁、构建辅助脚本和测试记录。当前 `main` 分支以稳定版 Linux v7.1.10 为目标，长期目标是逐步替代 vendor 内核。
...
默认要求 WSL2 中存在 Linux v7.1.10 源码检出：`/root/w132d-build/linux-v7.1.10`。如有需要，可通过环境变量覆盖路径。
...
This repository contains the public board description, patches, build helpers, and test notes for the ZTE Cloud Computer W132D (Rockchip RK3528). The active `main` branch targets Linux v7.1.10 from the stable series and is intended to replace the vendor kernel over time.
...
The helpers expect a Linux v7.1.10 checkout at `/root/w132d-build/linux-v7.1.10` inside WSL2 by default. Override paths with environment variables when needed.
