...
| Rockchip BSP 内核 | `https://github.com/armbian/linux-rockchip`，分支 `rk-6.1-rkr5` | RK3528 驱动支持的移植依据（eMMC HS400 tap 值、TSADC 码表与初始化、RKVDEC 匹配项）；`rk3528_codec.*` 与 `es7202.*` 是原封不动取回后按 7.1 ASoC API 适配的完整驱动 | 与 Linux 同源；`rk3528_codec.*` 为 GPL-2.0+（Rockchip），`es7202.*` 为 GPL-2.0（Everest），补丁采用 GPL-2.0-only |
...
| Rockchip BSP kernel | `https://github.com/armbian/linux-rockchip`, branch `rk-6.1-rkr5` | Reference for the RK3528 driver support (eMMC HS400 tap values, TSADC code table and init, RKVDEC match entry); `rk3528_codec.*` and `es7202.*` are complete drivers taken unmodified and then adapted to the 7.1 ASoC API | Same origin as Linux; `rk3528_codec.*` is GPL-2.0+ (Rockchip) and `es7202.*` is GPL-2.0 (Everest); patches are GPL-2.0-only |
