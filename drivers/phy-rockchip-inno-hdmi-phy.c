...
static int inno_hdmi_phy_clk_determine_rate(struct clk_hw *hw,
					    struct clk_rate_request *req)
{
	unsigned long parent_rate = req->best_parent_rate;
	long rate;

	rate = inno_hdmi_phy_clk_round_rate(hw, req->rate, &parent_rate);
	if (rate < 0)
		return rate;
...
static const struct clk_ops inno_hdmi_phy_clk_ops = {
	...
	.determine_rate = inno_hdmi_phy_clk_determine_rate,
