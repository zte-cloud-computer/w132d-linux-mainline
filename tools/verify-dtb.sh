...
# ## 为什么需要
#
# 板级描述来自上游 DTS（随 cache/upstream/w132d-port 的固定提交），本仓库只做
# 「去掉显示路径」这一处变换。但**上游改了 DTS**或**换了固定提交**都可能悄悄拿掉
# 某个关键属性，而 DTB 照样编得出来、照样是合法的树 —— 只是设备起不来，或者起来了
# 缺一半功能。上游 main 分支就有现成的例子：我们钉的那个合并点之后紧接着的提交
# 把 eMMC 从 HS400 降回了 52MHz。
#
# 每一条都对应一个真实后果：
